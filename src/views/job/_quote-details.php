 ...
 */
if (!empty($_GET['zdebug'])){
    //to test issue on production when local env was ready and issue was urgent
    require_once __DIR__ . '/_quote-details_debug.php';
    return;
}
?>
...
<div class="pdf">
    ...
    <table class="table table-condensed table-striped table-bordered" id="quote-table">
        ...
        <tr>
            ...
            <?php if (!$model->hideProductDiscounts()) { ?>
            ...
            <tr class="product<?= empty($itemDescriptions) ? ' bottom-border' : '' ?>">
                ...
                <?php if (!$model->hideProductPrices()) { ?>
                    <td class="text-right" style="width:80px;" rowspan="<?= $rowspan ?>">
                        <?php
                        $values = [];
                        $price = $model->hideProductDiscounts() ? $product->quote_factor_price - $product->quote_discount_price : $product->quote_factor_price;
                        $values[] = '$' . number_format($price * $model->quote_markup / $product->quote_quantity, 3);
                        if ($product->forkQuantityProducts) {
                            foreach ($product->forkQuantityProducts as $_product) {
                                $price = $model->hideProductDiscounts() ? $_product->quote_factor_price - $_product->quote_discount_price : $_product->quote_factor_price;
                                $values[] = '$' . number_format($price * $model->quote_markup / $_product->quote_quantity, 3);
                            }
                        }
                ...
                <?php if (!$model->hideProductDiscounts()) { ?>
                    <td class="text-right" style="width:80px;" rowspan="<?= $rowspan ?>">
                        <?php
                        $values = [];
                        $values[] = '$' . number_format($product->quote_discount_price * $model->quote_markup, 2);
                        if ($product->forkQuantityProducts) {
                            foreach ($product->forkQuantityProducts as $_product) {
                                $values[] = '$' . number_format($_product->quote_discount_price * $model->quote_markup, 2);
                            }
                        }
                        echo implode('<br>', $values);
                        ?>
                ...
                <?php if (!$model->hideProductPrices()) { ?>
                    <td class="text-right" style="width:80px;" rowspan="<?= $rowspan ?>">
                        <?php
                        $values = [];
                        $values[] = '$' . number_format(round($product->quote_factor_price * $model->quote_markup, 2) - round($product->quote_discount_price * $model->quote_markup, 2), 2);
                        if ($product->forkQuantityProducts) {
                            foreach ($product->forkQuantityProducts as $_product) {
                                $values[] = '$' . number_format(round($_product->quote_factor_price * $model->quote_markup, 2) - round($_product->quote_discount_price * $model->quote_markup, 2), 2);
                            }
                        }
                        echo implode('<br>', $values);
                        ?>
