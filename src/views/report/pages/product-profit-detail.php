...
$this->title = Yii::t('app', 'Product Profit Detail');
...
// set product type
$product_type_id = Y::GET('product_type_id');
        ...
        <div class="box-body">
            ...
            <div class="row">
                <div class="col-sm-3 col-md-3">
                    <?= Html::a('<i class="fa fa-arrow-left"></i> ' . date($paginateDateFormat, strtotime($from . ' -' . $nextFormat)), ['/report/index', 'report' => 'product-profit-detail', 'date' => date('Y-m-d', strtotime($from . ' -' . $nextFormat)), 'product_type_id' => $productType->id], ['class' => 'btn btn-default']); ?>
                </div>
                <div class="col-sm-6 col-md-6 text-center">
                    <h2 style="margin-top: 0"><?= $this->title . ' - ' . date($paginateDateFormat, strtotime($from)); ?></h2>
                    <h3><?= $productType->getBreadcrumbString() ?></h3>
                    <?= date('d-m-Y', strtotime($from)) ?> - <?= date('d-m-Y', strtotime($to)) ?>
                </div>
                <div class="col-sm-3 col-md-3 text-right">
                    <?= Html::a(date($paginateDateFormat, strtotime($from . ' +' . $nextFormat)) . ' <i class="fa fa-arrow-right"></i>', ['/report/index', 'report' => 'product-profit-detail', 'date' => date('Y-m-d', strtotime($from . ' +' . $nextFormat)), 'product_type_id' => $productType->id], ['class' => 'btn btn-default']); ?>
                </div>
            </div>
            ...
            <table class="table table-condensed table-bordered">
                ...
                foreach ($products->all() as $product) {
                    ...
                    ?>
                    <tr>
                        <td><?= $product->getLink($product->getTitle()) ?></td>
