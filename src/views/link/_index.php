...
$title = isset($title) ? $title : Yii::t('app', 'Links');
...
$relationName = isset($relationName) ? $relationName : 'links';
...
<div class="box">
        ...
        <?php if ($showActions) { ?>
            <div class="box-tools pull-right">
                <?= Html::a('<i class="fa fa-plus"></i> ' . Yii::t('app', 'Create'), [
                    '//link/create',
                    'Link' => ['model_name' => $modelName, 'model_id' => $modelId],
                    'ru' => $ru,
    ...
    <div class="box-body">
        <?php
        $sortables = [];
        foreach ($model->$relationName as $link) {
            $sortables[] = [
                'content' => $this->render('//link/_view', ['model' => $link, 'showActions' => $showActions]),
                'options' => [
                    'id' => 'Link_' . $link->id,
