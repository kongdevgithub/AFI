...
/**
 * Class JobArtworkEmailForm
 * @package app\models\form
 *
 * @property \app\models\Job $job
 */
class JobArtworkEmailForm extends Model
{
    ...
    public function send()
    {
        ...
        foreach ($this->job->products as $product) {
            foreach ($product->items as $item) {
                ...
                if ($status[1] == 'artwork' && $item->artwork) {
                    $item->status = $status[0] . '/approval';
                    if (!$item->save(false)) {
        ...
        EmailManager::sendArtworkApproval($this->job, $this->email_address);
        return true;
    }
