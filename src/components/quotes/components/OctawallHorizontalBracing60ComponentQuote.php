...
/**
 * OctawallHorizontalBracing60ComponentQuote
 */
class OctawallHorizontalBracing60ComponentQuote extends BaseComponentQuote
{
    ...
    public function getQuoteQuantity($component, $item, $options = [])
    {
        ...
        if ($size['width'] > 1500 && $size['width'] <= 3000 && $size['height'] > 1500 && $size['height'] <= 3000) {
            return $unitQuantity * (($size['width'] / 1000) - ($extrusionOffset / 1000 * 4));
        }
