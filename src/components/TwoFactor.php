...
/**
 * @see https://github.com/Voronenko/PHPOTP
 */
...
class TwoFactor extends Component
{
    ...
    public function barcode($safe = false)
    {
        $identity = Yii::$app->user->identity;
        if ($safe) {
            return Html::img(static::getBarCodeUrl($identity->username, $this->domain, $identity->two_factor['secret'], '', ''));
        }
        return Html::img(static::getBarCodeUrl($identity->username, $this->domain, $identity->two_factor['secret'], $this->issuer, $this->issuer));
    }

    public function check($code)
    {
        return static::verify(Yii::$app->user->identity->two_factor['secret'], $code);
    }
     ...
     */
    private static function verify($secretkey, $code, $rangein30s = 3)
    {
        $key = static::decode($secretkey);
    ...
    private static function getBarCodeUrl($username, $domain, $secretkey, $issuer, $prefix)
    {
        $url = "https://chart.apis.google.com/chart";
        $url .= "?chs=150x150&chld=M|0&cht=qr&chl=otpauth://totp/";
        if ($prefix) {
            $url .= rawurlencode($prefix) . "%3A%20";
        }
        $url .= $username . "%40" . $domain . "%3Fsecret%3D" . $secretkey;
        if ($issuer) {
