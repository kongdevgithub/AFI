...
/**
 * IlluminatedPowerEdgeField
 */
class IlluminatedPowerEdgeField extends ComponentField
{
    ...
    {
        $cacheKey = 'IlluminatedPowerEdgeField.autoFindComponent.' . $productToOption->id;
        ...
        foreach ($query->orderBy(['name' => SORT_ASC])->all() as $_component) {
            ...
            if (isset($componentConfig['led_power']['edge'])) {
                $ledPowers[$_component->id] = $componentConfig['led_power']['edge'];
            }
        }
        asort($ledPowers);
        ...
        $maxSize = $size ? max($size) : 0;
        $minSize = $size ? min($size) : 0;
        $component_id = false;
        $rows = 1;
        $ils80 = $productToOption->item_id ? $productToOption->item->getProductToComponent(Component::COMPONENT_ILS80) : $productToOption->product->getProductToComponent(Component::COMPONENT_ILS80);
        if ($minSize > 1500 || $minSize > 1000 && $ils80) {
            $rows = 2;
        }
        $powerCapacity = ceil($maxSize / 500) * $rows;
        $powerQuantity = 1;
